...
const CropEasy = ({ photo, setOpenCrop }) => {
  ...
  const { mutate, isLoading } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success("La foto del perfil está actualizada.");
    },
    onError: (error) => {
  ...
  return (
    <div className="fixed z-[1000] inset-0 bg-black/50 flex justify-center p-5 overflow-auto">
      <div className="bg-white h-fit w-full sm:max-w-[350px] p-5 rounded-lg">
        <h2 className="font-semibold mb-2">Recortar Imagen</h2>
        ...
        <div className="flex justify-between gap-2 flex-wrap">
          <button
            ...
            className="px-5 py-2.5 rounded-full text-[#FA5564] border border-[#FA5564] text-sm disabled:opacity-70"
          >
            Cancelar
          </button>
          <button
            ...
            className="px-5 py-2.5 rounded-full text-white bg-[#96C6D9] text-sm disabled:opacity-70"
          >
            Recortar y cargar
          </button>
        </div>
